...
export function createRemoteBridge(token: string): PreloadApi & NetworkPreloadApi & RemoteAccessPreloadApi {
  return {
    ...
    touchRecentProject: () => Promise.reject(new Error(NOT_LOCAL_MESSAGE)),
    ...
    startContainerExec: () => Promise.reject(new Error(NOT_YET_REMOTE_MESSAGE)),
    writeContainerExec: () => Promise.reject(new Error(NOT_YET_REMOTE_MESSAGE)),
    stopContainerExec: () => Promise.reject(new Error(NOT_YET_REMOTE_MESSAGE)),
    subscribeExecOutput: () => () => {},
    subscribeExecExit: () => () => {},
    getConfigDrift: (projectId) => postJson<ConfigDriftResult>("/api/config-drift", { projectId }, token),
    checkImageUpdate: (image) => postJson<CheckImageUpdateResult>("/api/check-image-update", { image }, token),
    backupVolume: () => Promise.reject(new Error(NOT_YET_REMOTE_MESSAGE)),
    restoreVolume: () => Promise.reject(new Error(NOT_YET_REMOTE_MESSAGE)),
